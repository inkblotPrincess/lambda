...
namespace lambda
{
    ...
    struct formatter : std::formatter<std::string>
    {
        ...
        {
            return std::formatter<std::string>::format(detail::ToStringCPO(Object), Context);
        }
    };
} // namespace lambda

template<class type>
concept can_format = requires(type Type) {{ lambda::detail::ToStringCPO(Type) } -> std::convertible_to<std::string>; };

template<can_format type>
struct std::formatter<type> : lambda::formatter<type>
{
